...
function run(){
  ...
  var laloRequest = new XMLHttpRequest();
    ...
    laloRequest.open(
      'GET', 
      'https://api.opencagedata.com/geocode/v1/json?q=' + place + '&key=ae8f7c0a9bb2421b9de7c3d34219d4e0&pretty=1'
    );
...
function task(div, poi){
  var detailsRequest = new XMLHttpRequest();
      var url = 'https://api.tomtom.com/search/2/poiDetails.json?key=SdTzSmdYJlkRfuldyzkawPAHe1SEQqOK&id=' + poi.id;
      detailsRequest.open(
        'GET',
        url
      );
    ...
    detailsRequest.onload = function(){
      ...
      if(this.status >= 200 && this.status < 400){
        if(Object.keys(data.result).length == 0){
          ...
          div.child(createSpan(data.result.description).class("desc"));
          photo = data.photos[0];
